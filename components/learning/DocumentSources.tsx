...
import { readJsonResponse } from "@/lib/utils/api-client";
...
export function DocumentSources({
  ...
  async function upload(
    ...
    try {
      ...
      const payload =
        await readJsonResponse<DocumentUploadResponse>(
          response,
          "Brick Tree received an unreadable document response from the server.",
        );
    ...
    <section className="source-uploader">
      <div className="section-heading-row compact">
        <div>
          ...
          <strong>
            Ground the graph in your material
          </strong>
        </div>
      ...
      {documents.length ? (
        <>
              ...
              (document) => (
                ...
                >
                  ...
                  <button
                    ...
                    aria-pressed={activeDocumentIds.has(
                      document.id,
                    )}
                    onClick={() =>
                      onToggleActive(
          ...
          <p className="source-selection-note">
            {sourceMode === "general"
              ? `${activeDocumentIds.size} source${
                  activeDocumentIds.size ===
                  1
                    ? ""
                    : "s"
                } selected · General mode does not use uploads for grounding.`
              : `${activeDocumentIds.size} of ${documents.length} uploaded source${
                  documents.length === 1
                    ? ""
                    : "s"
                } active for grounding.`}
          </p>

          {sourceMode !== "general" &&
          activeDocumentIds.size === 0 ? (
            <p className="source-warning">
              Select at least one uploaded
              source to use{" "}
              {sourceMode ===
              "uploaded-only"
                ? "Uploaded Only"
        ...
        <p className="muted">
          ...
          Markdown, CSV, TSV, or JSON.
          Files are parsed for this live
          session only; Brick Tree does
          not persist them on Vercel.
        </p>
      )}
