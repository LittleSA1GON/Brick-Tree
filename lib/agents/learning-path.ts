...
- Each new brick must name one or two exact source titles in connectsFrom. Those titles must come from currentLayerTitles when supplied. On an initial free-form raw statement, use the concise parsedFoundations titles instead.
...
Foundation behavior:
- The learner may provide arbitrary free-form prose. When Raw learner foundation statement is supplied, parse it into concise positively-known concepts in parsedFoundations. Exclude goals, uncertainty, and negated claims such as "I do not understand X".
- If raw prose is absent or cannot be parsed safely, parsedFoundations may be []. The runtime will use the deterministic Known learner foundation as a fallback.
- Keep parsedFoundations short, concept-like, and faithful to what the learner actually said; do not upgrade vague exposure into mastery.
- If the raw statement explicitly says the learner is starting from scratch and no positive known concepts can be extracted, return parsedFoundations as [] and use 1-3 foundationSuggestions for true beginner starting concepts appropriate to the learner profile.
...
Never invent evidence identifiers or URLs.`,
  ...
  buildUserPrompt(input) {
    ...
    const requestedCount = input.targetDirectionCount
      ...
      : `one more than the final visible foundation row (use parsedFoundations when raw prose was supplied; deterministic current ${currentLayer.length}, plus any permitted foundationSuggestions), maximum 10`;

    return `Current Brick row: ${currentLayer.slice(0, 20).join(", ")}
Known learner foundation: ${input.knownConcepts.slice(0, 20).join(", ")}
Raw learner foundation statement: ${input.rawKnowledgeInput?.slice(0, 3000) || "not supplied"}
