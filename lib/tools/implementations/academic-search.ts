...
export const academicSearchTool: AgentTool<z.infer<typeof InputSchema>, RawSearchResult[]> = {
  ...
  async execute(input, context) {
    const settled = await Promise.allSettled([
      searchCrossref(input.query, input.limit, context.signal),
      searchOpenAlex(input.query, input.limit, context.signal),
      searchSemanticScholar(input.query, input.limit, context.signal),
    ]);
    ...
    if (!results.length && settled.some((item) => item.status === "rejected")) {
      ...
      throw new Error(`Academic searches failed: ${messages.join(" | ")}`);
    }
